"""Static-threshold range peak detection (FMCW) — shared by plots and live OSC."""
...
class RangePeakDetectionConfig:
    ...
    # analysis / plot (post-processing only)
        ...
        return cls(
            ...
            secondary_min_range_sep_m=float(
                block.get(
                    ...
                    osc_legacy.get("secondary_min_range_sep_m", 0.3),
                )
            ),
            secondary_max_range_sep_m=float(
                block.get(
                    "secondary_max_range_sep_m",
                    osc_legacy.get("secondary_max_range_sep_m", 2.0),
                )
            ),
...
def detect_range_peaks_frame(
    ...
    """
    Static-threshold peak picker for one range profile.

    Returns ``(range_index, range_m, snr_db)`` sorted by SNR (highest first).
    """
    if profile.size == 0:
        return []
...
def osc_targets_from_profile(
    ...
    """
    OSC targets: strongest peak + optional second in ``secondary_*`` separation band.
    """
...
def detect_peaks_volume(
    ...
    """All peaks per frame for analysis plots / npz export."""
